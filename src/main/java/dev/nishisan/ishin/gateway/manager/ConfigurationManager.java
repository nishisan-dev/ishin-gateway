 ...
 */
package dev.nishisan.ishin.gateway.manager;
...
import dev.nishisan.ishin.gateway.configuration.EndPointURLContext;
import dev.nishisan.ishin.gateway.configuration.OauthServerClientConfiguration;
import dev.nishisan.ishin.gateway.configuration.SSLListenerConfiguration;
...
public class ConfigurationManager {
    ...
    private ArrayList<String> getConfigurationPaths() {
        ...
        paths.add("/app/ishin-gateway/config/adapter.yaml");
        paths.add("/etc/ishin-gateway/adapter.yaml");
        paths.add("c:/temp/inventory.yaml");
        String envPath = System.getenv("ISHIN_CONFIG");
        if (envPath != null) {
            //
...
//            EndPointURLContext endpointUrlContext = new EndPointURLContext("/*","ANY","default/Rules.groovy");
            ...
            defaultHttpBackend.getMembers().add(
                    new dev.nishisan.ishin.gateway.configuration.UpstreamMemberConfiguration("http://keycloak:8080"));
