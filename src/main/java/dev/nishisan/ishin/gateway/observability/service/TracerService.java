 ...
 */
package dev.nishisan.ishin.gateway.observability.service;
 ...
 * {@link DisposableBean}.
 * <p>
 * Cada instância do Ishin Gateway recebe um {@code instanceId} único que é composto
...
public class TracerService implements DisposableBean {
    ...
    public TracerService() {
        ...
        logger.info("Ishin Gateway instance ID: [{}]", this.instanceId);
    }

    /**
     * Resolve o Instance ID único desta instância do Ishin Gateway.
     * Ordem de precedência: env ISHIN_INSTANCE_ID → hostname → UUID.
     */
    private static String resolveInstanceId() {
        String envId = System.getenv("ISHIN_INSTANCE_ID");
    ...
    /**
     * Resolve o sample rate configurado.
     * <p>
     * Ordem de precedência: env {@code TRACING_SAMPLE_RATE} → system property
     * {@code tracing.sample.rate} → default {@code 1.0} (100%).
     * <p>
     * Valores válidos: {@code 0.0} a {@code 1.0}.
     */
    private float resolveSampleRate() {
        String envRate = System.getenv("TRACING_SAMPLE_RATE");
        if (envRate != null && !envRate.isBlank()) {
            try {
                return clampRate(Float.parseFloat(envRate.trim()));
            } catch (NumberFormatException e) {
                logger.warn("Valor inválido para TRACING_SAMPLE_RATE='{}', usando default 1.0", envRate);
            }
        }
        String sysProp = System.getProperty("tracing.sample.rate");
        if (sysProp != null && !sysProp.isBlank()) {
            try {
                return clampRate(Float.parseFloat(sysProp.trim()));
            } catch (NumberFormatException e) {
                logger.warn("Valor inválido para tracing.sample.rate='{}', usando default 1.0", sysProp);
            }
        }
        return 1.0f;
    }
    ...
    private Sampler buildSampler(float rate) {
        if (rate <= 0.0f) return Sampler.NEVER_SAMPLE;
        if (rate >= 1.0f) return Sampler.ALWAYS_SAMPLE;
    ...
    public Tracing getTracingInstance(String serviceName) {
        ...
        return tracingInstances.computeIfAbsent(serviceName, name -> {
            ...
            float sampleRate = resolveSampleRate();
            Sampler sampler = buildSampler(sampleRate);
            logger.info("Creating new Tracing instance for service: [{}] (sampleRate={})", qualifiedName, sampleRate);
            return Tracing.newBuilder()
                    .localServiceName(qualifiedName)
