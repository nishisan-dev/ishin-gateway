    ...
    static TunnelLoadBalancer forAlgorithm(String algorithm) {
        return switch (algorithm.toLowerCase()) {
            case "least-connections" ->
                    new dev.nishisan.ishin.gateway.tunnel.lb.LeastConnectionsBalancer();
            case "weighted-round-robin" ->
                    new dev.nishisan.ishin.gateway.tunnel.lb.WeightedRoundRobinBalancer();
            default -> // "round-robin" e qualquer valor desconhecido
                    new dev.nishisan.ishin.gateway.tunnel.lb.RoundRobinBalancer();
        };
    }
}
