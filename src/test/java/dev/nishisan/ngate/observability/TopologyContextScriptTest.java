...
import dev.nishisan.ngate.configuration.*;
...
class TopologyContextScriptTest {
    ...
    void setUp() {
        ...
        String tempDir = System.getProperty("java.io.tmpdir") + "/ngate-topo-test-" + System.nanoTime();
    ...
    void testGatewayNodePreserved() throws Exception {
        ...
        assertTrue(nodes.stream().anyMatch(n -> "gateway".equals(n.get("type")) && "ngate".equals(n.get("id"))),
                "Deve conter nó gateway 'ngate'");
    }
    ...
    @Test
    @Order(8)
    @DisplayName("Topologia inclui edge context → ngate")
    @SuppressWarnings("unchecked")
    void testContextToGatewayEdge() throws Exception {
        ...
        assertTrue(edges.stream().anyMatch(e ->
                        "context:http-noauth:default".equals(e.get("source"))
                                && "ngate".equals(e.get("target"))
                                && "inbound-context".equals(e.get("type"))),
                "Deve conter edge context → ngate");
    }
    ...
    void testOriginalEdgesPreserved() throws Exception {
        ...
        // listener → ngate (inbound) original
        assertTrue(edges.stream().anyMatch(e ->
                        "listener:http-noauth".equals(e.get("source"))
                                && "ngate".equals(e.get("target"))
        ...
        // ngate → backend (upstream) original
        assertTrue(edges.stream().anyMatch(e ->
                        "ngate".equals(e.get("source"))
