...
public class NGridClusterOAuthIntegrationTest {
    ...
    private static final ImageFromDockerfile ishinImage = new ImageFromDockerfile("ishin-oauth-test", false)
    ...
    // ─── Nó 1 (Ishin Gateway com OAuth) ─────────────────────────────────────────

    @Container
    private static final GenericContainer<?> node1 = new GenericContainer<>(ishinImage)
            .withNetwork(network)
            .withNetworkAliases("ishin-oauth-node1")
            .withEnv("SPRING_PROFILES_DEFAULT", "test")
            .withEnv("ISHIN_CONFIG", "/app/config/adapter-test-cluster-oauth.yaml")
            .withEnv("ISHIN_CLUSTER_NODE_ID", "ishin-oauth-node1")
            .withEnv("ISHIN_INSTANCE_ID", "oauth-node-1")
            .withCopyFileToContainer(
    ...
    // ─── Nó 2 (Ishin Gateway com OAuth) ─────────────────────────────────────────

    @Container
    private static final GenericContainer<?> node2 = new GenericContainer<>(ishinImage)
            .withNetwork(network)
            .withNetworkAliases("ishin-oauth-node2")
            .withEnv("SPRING_PROFILES_DEFAULT", "test")
            .withEnv("ISHIN_CONFIG", "/app/config/adapter-test-cluster-oauth.yaml")
            .withEnv("ISHIN_CLUSTER_NODE_ID", "ishin-oauth-node2")
            .withEnv("ISHIN_INSTANCE_ID", "oauth-node-2")
            .withCopyFileToContainer(
                    MountableFile.forClasspathResource("adapter-test-cluster-oauth.yaml"),
                    "/app/config/adapter-test-cluster-oauth.yaml")
            .withExposedPorts(PROXY_PORT, HEALTH_PORT, NGRID_PORT)
