...
/**
 * Testes de integração do Cluster Mode NGrid com 2 nós Ishin Gateway reais.
 * <p>
...
public class NGridClusterIntegrationTest {
    ...
    private static final ImageFromDockerfile ishinImage = new ImageFromDockerfile("ishin-test", false)
    ...
    private static final GenericContainer<?> node1 = new GenericContainer<>(ishinImage)
            .withNetwork(network)
            .withNetworkAliases("ishin-node1")
            .withEnv("SPRING_PROFILES_DEFAULT", "test")
            .withEnv("ISHIN_CONFIG", "/app/config/adapter-test-cluster.yaml")
            .withEnv("ISHIN_CLUSTER_NODE_ID", "ishin-node1")
            .withEnv("ISHIN_INSTANCE_ID", "test-node-1")
            .withCopyFileToContainer(
    ...
    private static final GenericContainer<?> node2 = new GenericContainer<>(ishinImage)
            .withNetwork(network)
            .withNetworkAliases("ishin-node2")
            .withEnv("SPRING_PROFILES_DEFAULT", "test")
            .withEnv("ISHIN_CONFIG", "/app/config/adapter-test-cluster.yaml")
            .withEnv("ISHIN_CLUSTER_NODE_ID", "ishin-node2")
            .withEnv("ISHIN_INSTANCE_ID", "test-node-2")
            .withCopyFileToContainer(
                    MountableFile.forClasspathResource("adapter-test-cluster.yaml"),
                    "/app/config/adapter-test-cluster.yaml")
            .withExposedPorts(PROXY_PORT, HEALTH_PORT, NGRID_PORT)
     ...
     * }
     * </pre>
     * Tenta também variantes do nome antigo (legacy).
     */
    private JsonNode getIshinDetails(JsonNode health) {
        JsonNode components = health.path("components");
        // Tentar variações possíveis do nome do HealthIndicator
        for (String key : new String[]{"ishinGateway", "IshinGateway", "ishin-gateway", "ishin"}) {
    ...
    void testClusterMeshFormation() {
        ...
        await().atMost(90, TimeUnit.SECONDS)
                ...
                .untilAsserted(() -> {
                    ...
                    JsonNode details1 = getIshinDetails(health1);
                    JsonNode details2 = getIshinDetails(health2);
    ...
    void testLeaderElection() {
        await().atMost(90, TimeUnit.SECONDS)
                .pollInterval(3, TimeUnit.SECONDS)
                .untilAsserted(() -> {
                    JsonNode details1 = getIshinDetails(getJson(healthUrl(node1)));
                    JsonNode details2 = getIshinDetails(getJson(healthUrl(node2)));
