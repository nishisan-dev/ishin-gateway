 ...
 * → HALF_OPEN → CLOSED (recovery). Usa Testcontainers com 1 nó Ishin Gateway e mock backends.
 * <p>
...
public class CircuitBreakerIntegrationTest {
    ...
    private static final ImageFromDockerfile ishinImage = new ImageFromDockerfile("ishin-cb-test", false)
    ...
    private static final GenericContainer<?> ishin = new GenericContainer<>(ishinImage)
            ...
            .withLogConsumer(new Slf4jLogConsumer(log).withPrefix("ishin-cb"));

    // ─── Helpers ──────────────────────────────────────────────────────────

    private String healthyProxyUrl() {
        return String.format("http://%s:%d/",
                ishin.getHost(), ishin.getMappedPort(PROXY_HEALTHY_PORT));
    }

    private String failingProxyUrl() {
        return String.format("http://%s:%d/",
                ishin.getHost(), ishin.getMappedPort(PROXY_FAILING_PORT));
    }
