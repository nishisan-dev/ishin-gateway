...
class ContextScriptMetricsTest {
    ...
    void testContextRequestCounter() {
        ...
        Counter counter = registry.find("ishin.context.requests.total")
        ...
        assertNotNull(counter, "Counter ishin.context.requests.total deve existir");
    ...
    void testContextRequestTimer() {
        ...
        Timer timer = registry.find("ishin.context.duration")
                .tag("listener", "http-noauth")
                .tag("context", "api-users")
                .tag("method", "POST")
                .timer();

        assertNotNull(timer, "Timer ishin.context.duration deve existir");
    ...
    void testContextErrorCounter() {
        ...
        Counter counter = registry.find("ishin.context.errors")
                .tag("listener", "http-noauth")
                .tag("context", "default")
                .tag("method", "GET")
                .counter();

        assertNotNull(counter, "Counter ishin.context.errors deve existir");
    ...
    void testScriptExecutionMetrics() {
        ...
        Counter counter = registry.find("ishin.script.executions.total")
                .tag("listener", "http-noauth")
                .tag("context", "default")
                .tag("script", "default/Rules.groovy")
                .counter();

        assertNotNull(counter, "Counter ishin.script.executions.total deve existir");
        assertEquals(2.0, counter.count());

        Timer timer = registry.find("ishin.script.duration")
                .tag("listener", "http-noauth")
                .tag("context", "default")
                .tag("script", "default/Rules.groovy")
                .timer();

        assertNotNull(timer, "Timer ishin.script.duration deve existir");
    ...
    void testScriptErrorCounter() {
        ...
        Counter counter = registry.find("ishin.script.errors")
                .tag("listener", "http-noauth")
                .tag("context", "default")
                .tag("script", "default/Rules.groovy")
                .counter();

        assertNotNull(counter, "Counter ishin.script.errors deve existir");
        assertEquals(1.0, counter.count());
    }
    ...
    void testDifferentContextsAreIsolated() {
        ...
        Counter counterA = registry.find("ishin.context.requests.total")
                .tag("context", "ctx-a")
                .counter();
        Counter counterB = registry.find("ishin.context.requests.total")
                .tag("context", "ctx-b")
                .counter();
    ...
    void testCollectorSeesNewMetrics() {
        ...
        boolean hasContextCounter = current.keySet().stream()
                .anyMatch(k -> k.startsWith("ishin.context.requests.total"));
        boolean hasContextTimer = current.keySet().stream()
                .anyMatch(k -> k.startsWith("ishin.context.duration"));
        boolean hasScriptCounter = current.keySet().stream()
                .anyMatch(k -> k.startsWith("ishin.script.executions.total"));
        boolean hasScriptTimer = current.keySet().stream()
                .anyMatch(k -> k.startsWith("ishin.script.duration"));

        assertTrue(hasContextCounter, "getCurrentMetrics deve conter ishin.context.requests.total");
        assertTrue(hasContextTimer, "getCurrentMetrics deve conter ishin.context.duration");
        assertTrue(hasScriptCounter, "getCurrentMetrics deve conter ishin.script.executions.total");
        assertTrue(hasScriptTimer, "getCurrentMetrics deve conter ishin.script.duration");
    }
    ...
    void testPersistenceInStorage() {
        ...
        assertTrue(names.stream().anyMatch(n -> n.startsWith("ishin.context.")),
                "Storage deve conter métricas ishin.context.*");
        assertTrue(names.stream().anyMatch(n -> n.startsWith("ishin.script.")),
                "Storage deve conter métricas ishin.script.*");
        ...
        List<DashboardStorageService.SeriesRecord> contextRecords =
                storage.queryMetrics("ishin.context.requests.total", from, to);
        assertFalse(contextRecords.isEmpty(),
                "Deve retornar registros de ishin.context.requests.total no histórico");

        // Timer: persistido como .mean, .max, .count
        List<DashboardStorageService.SeriesRecord> timerMeanRecords =
                storage.queryMetrics("ishin.context.duration.mean", from, to);
        assertFalse(timerMeanRecords.isEmpty(),
                "Deve retornar registros de ishin.context.duration.mean no histórico");

        List<DashboardStorageService.SeriesRecord> timerMaxRecords =
                storage.queryMetrics("ishin.context.duration.max", from, to);
        assertFalse(timerMaxRecords.isEmpty(),
                "Deve retornar registros de ishin.context.duration.max no histórico");
    }

    // ─── Helper: cria DashboardStorageService in-memory ─────────────────

    private DashboardStorageService createInMemoryStorage() {
        String tempDir = System.getProperty("java.io.tmpdir") + "/ishin-test-" + System.nanoTime();
