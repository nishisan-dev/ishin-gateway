 ...
 * {@code ishin_*} e que elas incrementam corretamente após tráfego.
 * <p>
 * Usa Testcontainers: 1 nó Ishin Gateway + 1 Nginx mock-backend.
 *
...
public class PrometheusMetricsIntegrationTest {
    ...
    private static final ImageFromDockerfile ishinImage = new ImageFromDockerfile("ishin-metrics-test", false)
    ...
    // ─── Ishin Gateway (standalone com métricas + CB) ────────────────────────────

    @Container
    private static final GenericContainer<?> ishin = new GenericContainer<>(ishinImage)
            .withNetwork(network)
            .withNetworkAliases("ishin-metrics")
            .withEnv("SPRING_PROFILES_DEFAULT", "test")
            .withEnv("ISHIN_CONFIG", "/app/config/adapter-test-metrics.yaml")
            .withEnv("ISHIN_INSTANCE_ID", "metrics-test-node")
            ...
            .withLogConsumer(new Slf4jLogConsumer(log).withPrefix("ishin"));

    // ─── Helpers ──────────────────────────────────────────────────────────

    private String prometheusUrl() {
        return String.format("http://%s:%d/actuator/prometheus",
                ishin.getHost(), ishin.getMappedPort(HEALTH_PORT));
    }

    private String proxyUrl() {
        return String.format("http://%s:%d/",
                ishin.getHost(), ishin.getMappedPort(PROXY_PORT));
    }
    ...
    @Test
    @Order(2)
    @DisplayName("T2: Após requests ao proxy, métricas ishin_requests_total incrementam")
    void testInboundMetricsIncrement() throws IOException {
        ...
        assertTrue(metrics.contains("ishin_requests_total"),
                "Should contain ishin_requests_total counter. Full metrics (first 2000 chars): " +
                metrics.substring(0, Math.min(2000, metrics.length())));

        log.info("✅ ishin_requests_total presente após tráfego");
    }
    ...
    void testUpstreamMetricsPresent() throws IOException {
        ...
        assertTrue(metrics.contains("ishin_upstream_requests_total"),
                "Should contain ishin_upstream_requests_total counter");

        assertTrue(metrics.contains("ishin_upstream_duration"),
                "Should contain ishin_upstream_duration timer");

        assertTrue(metrics.contains("ishin_request_duration"),
                "Should contain ishin_request_duration timer");

        log.info("✅ Métricas upstream presentes: ishin_upstream_requests_total, ishin_upstream_duration, ishin_request_duration");
    }
