# Brainstorm: Escalabilidade Horizontal do ishin-gateway
...
- **Footprint do NGrid dentro do ishin-gateway**: Gossip + heartbeat + replicação impacta o hot path do proxy?
...
- O `ishin-cli rules deploy` é o **único ponto de entrada** para aplicar regras.
...
| `ishin-cli` | Script/jar para deploy de rules |
| Integração NGrid | `DistributedMap` + listener para aplicação em followers |

### 3.7 Autenticação do Admin API ⚠️

O endpoint `/admin/*` e o `ishin-cli` **devem** ser autenticados. Opções avaliadas:
...
- **Necessidade:** Obrigatório para LB, observabilidade operacional, e base para o `ishin-cli`.
...
```
StatsUtils.notifyHitCounter("ishin.req.listener.http")
...
| Métrica | Tipo | Via |
|---|---|---|
| `ishin.requests.total{listener}` | Counter | `notifyHitCounter` |
| `ishin.request.duration.ms{listener}` | Average | `notifyAverageCounter` |
| `ishin.request.errors{listener}` | Counter | `notifyHitCounter` |

**Per Upstream (backend):**
| Métrica | Tipo | Via |
|---|---|---|
| `ishin.upstream.requests{backend}` | Counter | `notifyHitCounter` |
| `ishin.upstream.duration.ms{backend}` | Average | `notifyAverageCounter` |
| `ishin.upstream.status{backend,code}` | Counter | `notifyHitCounter` |

**Per Rule (hitcount):**
| Métrica | Tipo | Via |
|---|---|---|
| `ishin.rule.hits{script}` | Counter | `notifyHitCounter` |
| `ishin.rule.duration.ms{script}` | Average | `notifyAverageCounter` |

**Cluster (quando NGrid ativo):**
| Métrica | Tipo | Via |
|---|---|---|
| `ishin.cluster.active.members` | Gauge | `notifyCurrentValue` |
| `ishin.cluster.is.leader` | Gauge (0/1) | `notifyCurrentValue` |
| `ishin.cluster.replication.lag` | Gauge | `notifyCurrentValue` |
| `ishin.rules.bundle.version` | Gauge | `notifyCurrentValue` |
...
| 6 | Rules Deploy (CLI + Bundle + replicação) | 🟡 Média | Médio | ✅ **Implementado** (Sessão 4) | Depende de #4 para cluster; funciona standalone sem. Todos os testes T8-T11 ✅ |
| 7 | Auth do Admin API (API Key para MVP) | 🟡 Média | Baixo | ✅ **Implementado** (Sessão 4) | Todos os testes T10/T11 passando ✅ |
...
- Instance ID: `ISHIN_INSTANCE_ID` env → hostname → UUID random
...
| `IshinGatewayHealthIndicator.java` | **[NEW]** HealthIndicator customizado (config + instanceId) |
...
- Sem `cluster:`, ishin-gateway roda 100% standalone (zero impacto)
...
- Porta NGrid default: `7100` (env: `ISHIN_CLUSTER_PORT`)
- Node ID: config → env `ISHIN_CLUSTER_NODE_ID` → hostname → UUID random
...
| `IshinGatewayHealthIndicator.java` | Cluster info: `clusterMode`, `isLeader`, `activeMembers`, `clusterNodeId` |
...
| — | Testes de integração Docker (Testcontainers + 2 nós ishin-gateway em cluster) | ✅ Completo (4/4 passando) |
...
- `RulesBundleManager` — @Service que gerencia lifecycle completo:
  ...
  - Publica no `DistributedMap("ishin-rules")` para replicação cluster
...
|---|---|---|---|
| T8 | Standalone rules deploy (POST multipart) | ✅ Passa | Multipart habilitado + readiness wait |
| T9 | Cluster replication (deploy nó 1 → nó 2 via DistributedMap) | ✅ Passa | Deploy v3 replicado em ~5s |
| T10 | Auth rejeita request sem API Key | ✅ Passa | — |
| T11 | Admin API rejeita payload vazio/inválido | ✅ Passa | — |

**Causa raiz T8/T9 (corrigida):**
- Undertow requer `spring.servlet.multipart.enabled=true` explicitamente — sem isso, fecha conexão no upload multipart
- `RulesBundleManager.onStartup()` fazia lookup hard do `ClusterService` — agora com try/catch para graceful standalone fallback
- Race condition de startup: `ApplicationReadyEvent` podia não ter completado antes do teste enviar requests
...
| `AdminController.java` | REST controller com auth X-API-Key + MultipartException handler |
...
| `application.properties` | Multipart upload habilitado com limites |
| `application-test.properties` | Multipart upload habilitado |
...
201953f fix: enable multipart upload for Undertow (required for admin rules deploy)
f3334ac fix: add MultipartException handler in AdminController for graceful 400 responses
fb38c74 fix: safe ClusterService lookup in RulesBundleManager — graceful standalone fallback
d45a54b fix: add Admin API readiness wait in T8 test to prevent startup race condition
```

**Build + Testes:** `mvn test -Dtest=NGridClusterRulesDeployIntegrationTest` ✅ (4/4 passando, 87.6s)
...
- `ProxyMetrics.java` — `@Component` centralizado com counters/timers Micrometer:
  - `ishin.requests.total` (listener, method, status) — total inbound requests
  - `ishin.request.duration` (listener, method) — timer inbound
  - `ishin.request.errors` (listener, method) — counter de erros inbound
  - `ishin.upstream.requests` (backend, method, status) — total upstream requests
  - `ishin.upstream.duration` (backend, method) — timer upstream
  - `ishin.upstream.errors` (backend, method) — counter de erros upstream
...
- `ClusterService` — Gauges Micrometer registrados após startup do NGridNode:
  - `ishin.cluster.active.members` — número de membros ativos
  - `ishin.cluster.is.leader` — 1=leader, 0=follower
...
| T2 | `ishin_requests_total` incrementa após tráfego | ✅ |
